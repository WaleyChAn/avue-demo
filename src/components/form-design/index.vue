<template>
  <div class="form-designer">
    <el-container>
                ...
                <template v-for="(item, cIndex) in field.list">
                  <li class="field-label"
                    ...
                    <a @click="handleFieldClick(item)">
                      <i :class="item.icon"></i>
                      <span>{{
                        item.title || item.label || item.standbyLabel
                      }}</span>
                    </a>
                  </li>
                </template>
      ...
      <el-container class="widget-container"
        ...
        <el-header class="widget-container-header">
          ...
          <div style="display: flex; align-items: center;">
            ...
            <el-button type="text"
                       size="medium"
                       @click="updateLanEnvironment">{{ lanEnvironment }}</el-button>
        ...
        <el-main :style="{
          ...
          <!-- {{ widgetForm }} -->
          <i class="eig-step-icon icon-chat"></i>
      ...
      <el-drawer title="预览"
                 class="of-form-design-drawer"
                 :with-header="false"
                 :visible.sync="previewVisible"
                 size="100%"
                 append-to-body
                 :before-close="handleBeforeClose"
                 @closed="handleDesignPreviewClosed">
        <design-preview :widgetForm="widgetForm"
                        @closeDialog="closeEditContactDialog()"
                        ref="formPreview"></design-preview>
        <!-- <div class="drawer-container">
          <div class="drawer-header">
            {{ form }}
            <h2 class="title">中国广核电力股份有限公司发文（表单标题）</h2>
          </div>
          <div class="drawer-body">
            ...
            >
              <template v-for="group in groups" :slot="`${group.prop}Header`">
                <component
                  v-if="group.aggregation"
                  :key="group.prop"
          ...
          <div class="drawer-footer clearfix">
            <el-button type="primary" class="right" @click="handlePreviewSubmit"
              >确定</el-button
            >
            <el-button class="right" @click="handleBeforeClose">取消</el-button>
          </div>
        </div> -->
      </el-drawer>
...
import { validatenull, removeDisabled, customValidate } from "./utils/index";
...
export default {
  ...
  data () {
    return {
      reload: Math.random(),
      lanEnvironment: sessionStorage.getItem("lanEnvironment")
        ? sessionStorage.getItem("lanEnvironment")
        : "zh_CN",
      widgetEmpty,
      fields,
      widgetForm: {
  ...
  methods: {
    handleDesignPreviewClosed () {
      this.$refs.formPreview.show = false
    },
    ab () {
      alert("ab");
    },
    ...
    async handleLoadStorage () {
      if (this.storage) {
        ...
        if (form) {
          this.transAvueOptionsToFormDesigner(JSON.parse(form)).then(data => {
            console.log("转化后的data", data);
        ...
        this.transAvueOptionsToFormDesigner(options).then(data => {
          console.log("转化后的data", data);
          this.widgetForm = data;
        });
    ...
    handlePreview () {
      if (!this.widgetForm.column || this.widgetForm.column.length == 0)
        ...
        this.$nextTick(() => {
          this.$refs.formPreview.convertForm();
        });
    ...
    getPlaceholder (item) {
      ...
      const enLabel = item.enLabel ? item.enLabel : "";
      const placeholder = item.placeholder;
      const enPlaceholder = item.enPlaceholder;
      if (placeholder && this.lanEnvironment == "zh_CN") {
        return `${placeholder}`;
      } else if (enPlaceholder && this.lanEnvironment == "en_US") {
      ...
      if (
        ...
        if (this.lanEnvironment == "zh_CN") {
          return `请选择 ${label}`;
        } else if (this.lanEnvironment == "en_US") {
          return `Please select ${enLabel}`;
        }
      } else {
        if (this.lanEnvironment == "zh_CN") {
          return `请输入 ${label}`;
        } else if (this.lanEnvironment == "en_US") {
          return `Please input ${enLabel}`;
        }
      }
    },
    ...
    updateLanEnvironment () {
      if (this.lanEnvironment == "zh_CN") {
        this.lanEnvironment = "en_US";
      } else {
        this.lanEnvironment = "zh_CN";
      }
      sessionStorage.setItem("lanEnvironment", this.lanEnvironment);
    },

    ruleControl (key1, item) {
      let column = this.findObject(this.myArray1.column, key1);
      // column.control = eval(this.ruleString(item));
      console.log("找到cloumn", column);
    },
    ruleString (item) {
      let rerurnIf = "";
      item.forEach(ru => {
        rerurnIf += `if (val ${ru.rule} ){
          returnString += "${ru.affectedProp} : {${ru.updateKey} : ${ru.trueValue}},"
        }else{
          returnString += "${ru.affectedProp} : {${ru.updateKey} : ${ru.falseValue}},"
        }
       `;
      });
      return `(val, form) => {
